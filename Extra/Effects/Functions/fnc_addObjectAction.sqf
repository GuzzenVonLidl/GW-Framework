...
params ["_box","_actionName","_canExplode",["_delay", 7],"_deleteOnComplete"];
...
_box addAction [_actionName, {
	...
	if (GVARMAIN(mod_ACE3)) then {
		[((_this select 0) getVariable QGVAR(delay)), _this, {	// On Complete
			(_this select 0) params ["_box"];
			_box setVariable [QGVAR(blockInventory), false, true];

			if (_box getVariable QGVAR(deleteOnComplete)) then {
				deleteVehicle _box;
			};

			[QGVAR(objectAction), []] call CBA_fnc_serverEvent;
