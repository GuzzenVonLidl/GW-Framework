...
if (VCOMAI_CurrentlySuppressing < VCOMAI_CurrentlySuppressingLimit) then
{
	VCOMAI_CurrentlySuppressing = VCOMAI_CurrentlySuppressing + 1;
	_TimeShot = _unit getVariable "VCOMAI_FiredTime";
	if ((diag_tickTime - _TimeShot) > 25) then
	{
		...
		_unit setVariable ["VCOMAI_FiredTime",diag_tickTime,true];
...

		{
				...
				if (_CheckDistance < 4 && (_Kn > 3.5)) then
				{
					...
					if (VCOMAI_Adrenaline) then
					{
						_x setAnimSpeedCoef VCOMAI_AdrenalineVar;
						_x spawn {sleep 8; _this setAnimSpeedCoef 1;};
					};
					if (VCOMAI_AIDEBUG isEqualTo 1) then
					{
						[_x,"I am suppressed!",30,20000] remoteExec ["VCOMAI_3DText",0];
					};
