//	[MortarName, side, "Firing Mode", "round type", ["position", inaccuracy], minimum range, maximum range] execVM "Scripts\NEKY_Mortars\NEKY_Mortars.sqf";
////////////
//	
//	1. MortarName
...
//	8. Look in to NEKY_Settings to tweak settings to your taste.
//
//////////
//	Usage
//////////
...
/////////////
//	
//	null = [this, west, "precise", "Medium", ["morTarget", 50],150,1400] execVM "Scripts\NEKY_Mortars\NEKY_Mortars.sqf";
//	null = [Mortar1, west, "Barrage", "Medium", ["auto", 30],100,500] execVM "Scripts\NEKY_Mortars\NEKY_Mortars.sqf";
...
Private ["_EnableMarking","_Dir","_ScreenSize","_NewInaccuracy","_ScreenReloadTime","_TempPos","_Roll","_GuidedSize","_GuidedReloadTime","_GuidedInaccuracyMultiplier""_SoundOn","_In","_Sound","_SoundTypes","_Marking","_FriendlyNear","_Danger","_DangerClose","_SelectedFiringMode","_SporadicReloadTime","_PreciseReloadTime","_BarrageReloadTime","_Units","_Scanner","_Gunner","_MinRange","_MaxRange","_Zone","_Unit","_Lock","_Group","_Avoid","_OffMap","_RandomFiringMode","_Temp","_This","_Side","_Position","_FiringMode","_BarrageSize","_PreciseSize","_SporadicSize","_Light","_Medium","_Heavy","_Smoke","_Flare","_TravelTime","_MarkSmoke","_MarkFlare","_RandomFiringMode","_Duration"];
...
if ( (isNil "_Mortar") or (TypeName _Mortar == "STRING") ) then 
{
	...
	if (Alive _Mortar) then 
	{
		if (_Mortar EmptyPositions "Gunner" > 0) then
		{
			...
			if (_Danger) then 
			{
				[_Unit, _DangerClose, _Mortar] spawn
				{
					Private ["_Unit","_Zone","_DangerClose","_x","_Mortar"];
			
					_Unit = (_This select 0);
					_DangerClose = (_This select 1);
					_Mortar = (_This select 2);
					
					// Create Scan Zone
					_Zone = CreateTrigger ["EmptyDetector",(getPosATL _Unit),false];
					_Zone SetTriggerActivation ["ANY", "PRESENT", False];
					_Zone SetTriggerArea [_DangerClose, _DangerClose, 0, false];
					WaitUntil { sleep 5; !(Alive _Unit) or {!((side _x) == (side _Unit)) && ((side _Unit) GetFriend (side _x) < 0.6) && (_Unit distance _x <= _DangerClose) && (((getposATL _x) select 2) < 5)} count (list _Zone) > 0};
...
NEKY_MortarAISequence = 
{
	...
	_Unit doArtilleryFire [_Position, _Mag, 1];
};
...
While {((Alive _Mortar) && (Alive _Unit) && (_Unit in _Mortar)) or (_OffMap)} do
{
	...
	if (_Scanner) then 
	{
		...
		if (_ScanVehicles) then
		{
			{ if ( !((side _x) == (side _Unit)) && ((side _Unit) GetFriend (side _x) < 0.6) && (_Unit distance _x > _MinRange) && (_Unit distance _x < _MaxRange) && (Alive _x) && (((getposATL _x) select 2) < 5) ) then {_Units PushBack _x}; } ForEach (list _Zone);
		} else {
			{ if ( !((side _x) == (side _Unit)) && ((side _Unit) GetFriend (side _x) < 0.6) && (_Unit distance _x > _MinRange) && (_Unit distance _x < _MaxRange) && (_x isKindOf "CAManBase") && (Alive _x) && (((getposATL _x) select 2) < 5) ) then {_Units PushBack _x}; } ForEach (list _Zone);
		};
	...
	if !(_FriendlyNear) then 
	{
		...
		switch (_SelectedFiringMode) do
		{
			...
			case "sporadic": 
			{
				...
				while {( !(_Index == _Count) && ((_OffMap) or ((Alive _Mortar) && (Alive _Unit) && (_Unit in _Mortar))) )} do
				{
