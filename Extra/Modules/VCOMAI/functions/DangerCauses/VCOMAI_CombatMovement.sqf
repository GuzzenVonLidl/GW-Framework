...
private ["_Unit", "_VCOMAI_MovedRecently", "_VCOMAI_VisuallyCanSee", "_NearestEnemy", "_intersections"];

	_Unit = _this select 0;
	_VCOMAI_MovedRecently = _this select 1;
	_VCOMAI_VisuallyCanSee = _this select 2;
	...
	//if (isNil "_NearestEnemy" || {_VCOMAI_MovedRecentlyCover} || {(typeName _NearestEnemy isEqualTo "ARRAY")} || {isNil "_Unit"} || {!(alive _NearestEnemy)} || {(_NearestEnemy distance _Unit) > 5000}) exitWith {};
	if (isNil "_NearestEnemy" || {(typeName _NearestEnemy isEqualTo "ARRAY")} || {isNil "_Unit"} || {!(alive _NearestEnemy)} || {(_DistanceCheck) > 2000}) exitWith {_Unit forcespeed -1;};
		...
		if (_ReturnedFriendly distance _Unit > 30 && !(_ReturnedFriendly isEqualTo [0,0,0])) then
		{
			...
			{
				[_Unit,"I wandered too far :(. Returning to group.",15,20000] remoteExec ["VCOMAI_3DText",0];
			};
	...
	{
			_VCOMAI_VisuallyCanSee = true;
			_Unit forceSpeed 0;
			_Unit setUnitPos "AUTO";
			_Unit doSuppressiveFire _NearestEnemy;
			if (VCOMAI_AIDEBUG isEqualTo 1) then
			{
				[_Unit,"Enemy is close! Fire fire fire!",15,20000] remoteExec ["VCOMAI_3DText",0];
			};
			_VCOMAI_VisuallyCanSee

	};


	if (_VCOMAI_MovedRecentlyCover) exitwith {};
	...
	if (_VCOMAI_MovedRecently) exitWith {};
	...
	{
			_VCOMAI_VisuallyCanSee = false;
			//_Unit spawn {sleep 10;if !(_Unit getVariable "VCOMAI_VisuallyCanSee") then {_Unit forceSpeed -1;};};
	};
//};