...
if (_Vehicle in VCOMAI_ArtilleryArray) exitWith {};
...
//Check if unit somehow is in the VCOMAI_ArtilleryArray and remove them.  This can happen to units who were inside artillery pieces but ejected.
	if (_Vehicle in VCOMAI_ArtilleryArray) then
	{
		VCOMAI_ArtilleryArray = VCOMAI_ArtilleryArray - [_Vehicle];
	};
...
if (_ArtyScan isEqualTo 1) then
{
	...
	//VCOMAI_ArtilleryArray = VCOMAI_ArtilleryArray + [_Vehicle];
	VCOMAI_ArtilleryArray pushBack _Vehicle;
	//null = [_Unit] execFSM "\VCOMAI_AI\AIBEHAVIORARTY.fsm";
	if (VCOMAI_AIDEBUG isEqualTo 1) then
	{
		[_Vehicle,"I am Artillery :D!!!!",500,20000] remoteExec ["VCOMAI_3DText",0];
	};
};
