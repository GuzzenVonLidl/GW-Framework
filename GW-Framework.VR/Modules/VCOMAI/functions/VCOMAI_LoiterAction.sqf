...
While {_Unit getvariable ["VCOMAI_LOITERING",true] && alive _Unit} do
{
	_CurrentAction = _Unit getvariable ["VCOMAI_LOITERINGACT",0];
	...
	switch (_RandomAction) do
	{
			case 1:
			{
				...
				//Get random position
				_Unit setVariable ["VCOMAI_LOITERINGACT",1];
				_rnd = random 10;
				_dist = (_rnd + 5);
				_dir = random 360;
				...
				[_Unit,_RandomAnimationList] remoteExec ["VCOMAI_playMoveEverywhere",0];
			};
			case 2:
			{
				_Unit setVariable ["VCOMAI_LOITERINGACT",2];
				_ClosestUnit = [(_UnitGroup - [_Unit]),_Unit] call VCOMAI_ClosestObject;
				_ClosestUnit setVariable ["VCOMAI_LOITERINGACT",2];
				...
				[_Unit,"Acts_B_hub01_briefing"] remoteExec ["VCOMAI_switchMoveEverywhere",0];
				sleep 2;
				[_ClosestUnit,"Acts_Kore_TalkingOverRadio_loop"] remoteExec ["VCOMAI_switchMoveEverywhere",0];
				sleep 80;
				[_Unit,""] remoteExec ["VCOMAI_switchMoveEverywhere",0];
				[_ClosestUnit,""] remoteExec ["VCOMAI_switchMoveEverywhere",0];
			...
			case 3:
			{
				_Unit setVariable ["VCOMAI_LOITERINGACT",3];
				_ClosestUnit = [(_UnitGroup - [_Unit]),_Unit] call VCOMAI_ClosestObject;
				_ClosestUnit setVariable ["VCOMAI_LOITERINGACT",3];
				...
				_ClosestUnit spawn
				{
					...
					While {_Counter < 11} do
					{
						...
						[_this,_RandomAnimationList] remoteExec ["VCOMAI_playMoveEverywhere",0];
						_Counter = _Counter + 1;
						sleep 10;
					};
				};
			...
			case 4:
			{
				...
				//Get random position
				_Unit setVariable ["VCOMAI_LOITERINGACT",4];
				_rnd = random 10;
				_dist = (_rnd + 5);
				_dir = random 360;
				_UnitPosition = getposworld _Unit;
				...
				WaitUntil {(_Unit distance _positions) < 2};
				sleep 2;
				[_Unit,"SitDown"] remoteExec ["VCOMAI_playActionNowEverywhere",0];
				sleep 100;
				[_Unit,"walkf"] remoteExec ["VCOMAI_playActionNowEverywhere",0];
