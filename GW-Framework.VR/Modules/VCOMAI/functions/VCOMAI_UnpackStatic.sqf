private ["_VCOMAI_Indoor","_cansee","_position", "_Unit", "_weapon", "_leader", "_gunner", "_assistant", "_UnitGroups", "_CurrentBackPack", "_class", "_parents", "_rnd", "_dist", "_dir", "_positions", "_myNearestEnemy", "_StaticClassName", "_IsMortar", "_WeaponClassname", "_StaticCreated", "_dirTo"];_group = 		_this select 0;
_position =		_this select 1;
_Unit = _this select 2;
_VCOMAI_HASDEPLOYED = _this select 3;
_weapon = 		objNull;
...
//_assistant action ["PutBag",_assistant];
	...
	_StaticClassName = _Unit getVariable "VCOMAI_StaticClassName";
	...
	else
	{
		...
		{
			if (_x isKindof "Building") then {_VCOMAI_Indoor = true;};
		} foreach _Array;
	...
	if (!(_cansee > 0) || (_VCOMAI_Indoor)) exitwith {};
