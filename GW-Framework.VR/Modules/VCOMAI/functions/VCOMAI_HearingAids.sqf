...
if !(_Return) then
{
	...
	_TimeShot = _unit getVariable "VCOMAI_FiredTimeHearing";
	...
	if ((diag_tickTime - _TimeShot) > 20) then
	{
		...
		{
			if ((_x distance _unit) < VCOMAI_HEARINGDISTANCE && !(_x getVariable "VCOMAI_ShotsFired") && (count ((group _Unit) call VCOMAI_Waypointcheck)) <= 0) then
			{
				...
				{
					[_x,"What was that? +0.5 to knowsAbout",10,20000] remoteExec ["VCOMAI_3DText",0];
				};
			};
		} foreach _Array1;
...
else
{
				...
				if (VCOMAI_AIDEBUG isEqualTo 1) then
				{
					[_unit,"I am sneaky snake...",10,20000] remoteExec ["VCOMAI_3DText",0];
				};
