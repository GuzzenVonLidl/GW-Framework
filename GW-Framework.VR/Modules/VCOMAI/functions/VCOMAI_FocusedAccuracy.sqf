...
_VCOMAI_DiagLastCheck = _this select 1;
_TargetPosition = _this select 2;

_VCOMAI_DiagLastCheck = diag_ticktime;
...
else
{
	...
	if (_TargetPosition distance _NewPosition < 30) then
	{
		...
		if (VCOMAI_RainImpact) then
		{
			_WeatherCheck = (rain)/VCOMAI_RainPercent;
		...
		{
			[_Unit,"Target has not moved...Increasing accuracy :D",15,20000] remoteExec ["VCOMAI_3DText",0];
		};
	}
	else
	{
		...
		if (VCOMAI_AIDEBUG isEqualTo 1) then
		{
			[_Unit,"Target has moved...Reset Accuracy :<",15,20000] remoteExec ["VCOMAI_3DText",0];
		};
	};
	_TargetPosition = _NewPosition;
};

_ReturnedArray = [_VCOMAI_DiagLastCheck,_TargetPosition];
