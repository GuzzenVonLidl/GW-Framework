...
private ["_Index","_FuelHP","_EngineHP","_HullHP","_TurretHP","_GunHP","_MainRotorHP","_atrqHP","_InstrumentsHP","_CarHullHP","_LeftTrackHP","_RightTrackHP","_Parts","_PartNames","_PartsHP","_Part","_PartName","_PartHP","_Partname","_Wheels","_y","_State","_index"];
...
Switch (true) do {
	...
	case (_Veh isKindOf "PLANE"): {
		...
		_PartNames = ["Hull"];
		_PartsHP = [_HullHP];
		GVAR(RepairingSpeed) = GVAR(RepairingSpeedPlane);
	};
};
...
for "_i" from 1 to (count _Parts) do {
	...
	GVAR(InspectionSpeed) = (GVAR(InspectionSpeed) / 3);
	[(format ["Inspecting %1.",_Partname]), _Veh] spawn FUNC(Hints);
	Sleep GVAR(InspectionSpeed);
	[(format ["Inspecting %1..",_Partname]), _Veh] spawn FUNC(Hints);
	Sleep GVAR(InspectionSpeed);
	[(format ["Inspecting %1...",_Partname]), _Veh] spawn FUNC(Hints);
	Sleep GVAR(InspectionSpeed);
	...
	if (_PartHP >= 0.01) then {
		While {_PartHP > 0.01 && (_Veh in GVAR(Actions))} do {
			if (_Veh isKindOf "TANK" and _Index >= 4) then {GVAR(RepairingSpeed) = GVAR(RepairingTrackSpeed);};
			if (!(_Veh in GVAR(Actions))) exitWith {[_SS,true] call FUNC(Available)};
			[(format ["Repairing %1.",_PartName]), _Veh] spawn FUNC(Hints);
			if (_Part == "Karoserie") then {_Veh setHit [_Part, (_PartHP - GVAR(RepairingSpeed))];} else {_Veh setHitPointDamage [_Part, (_PartHP - GVAR(RepairingSpeed))];};
			if (_Part == "Karoserie") then {_PartHP = _Veh gethit _Part} else {_PartHP = _Veh gethitpointdamage _Part};
			sleep 0.33;
			[(format ["Repairing %1..",_PartName]), _Veh] spawn FUNC(Hints);
			_Veh setHitPointDamage [_Part, (_PartHP - GVAR(RepairingSpeed))];
			if (_Part == "Karoserie") then {_PartHP = _Veh gethit _Part} else {_PartHP = _Veh gethitpointdamage _Part};
			sleep 0.33;
			[(format ["Repairing %1...",_PartName]), _Veh] spawn FUNC(Hints);
			_Veh setHitPointDamage [_Part, (_PartHP - GVAR(RepairingSpeed))];
