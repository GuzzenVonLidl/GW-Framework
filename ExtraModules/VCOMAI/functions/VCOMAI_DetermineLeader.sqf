...
if (_GroupLeader isEqualTo _Unit) then {_VCOMAI_GroupLeader = true;} else {_VCOMAI_GroupLeader = false;};

_SubLeader = isFormationLeader _Unit;
if (_SubLeader) then {_VCOMAI_SubLeader = true;} else {_VCOMAI_SubLeader = false;};


_Leader = _VCOMAI_GroupLeader;
if (isNil "_Leader") exitWith {};
_SubLeader = _VCOMAI_SubLeader;
