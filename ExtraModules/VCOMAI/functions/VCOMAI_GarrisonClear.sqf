...
_VCOMAI_InCover = _this select 2;
_VCOMAI_ActivelyClearing = _this select 3;
_VCOMAI_StartedInside = _this select 4;
_VCOMAI_GARRISONED = _this select 5;

//Function to send AI to clear buildings
if (_VCOMAI_MovedRecentlyCover || {_VCOMAI_ActivelyClearing} || {_VCOMAI_StartedInside} || {_VCOMAI_GARRISONED}) exitWith {};
...
//Stop the AI - and then tell them to move to the house
{
	...
	{
		[_Locations,_x,_VCOMAI_InCover,_VCOMAI_ActivelyClearing,_Enemy] spawn VCOMAI_GarrisonClearPatrol;
	};
